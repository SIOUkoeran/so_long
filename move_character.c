...
/*   Updated: 2022/05/14 20:50:17 by mkim3            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_key_event(int key, t_game_info *game_info)
{
	...
	if (key == W)
	{
		if (map[game_info->x - 1][game_info->y])
			move(game_info, game_info->x - 1 , game_info->y);
	}
	else if (key == A)
	{
		if (map[game_info->x][game_info->y - 1])
...
static void	ft_free_game_info(t_game_info *game_info)
{
	int	i;
...
void	move(t_game_info *game_info, int next_x, int next_y)
{
	...
	else if (map[next_x][next_y] == 'E')
		if (game_info->item_cnt == 0)
		{
			printf("이동 회수 : %d\n", game_info->walks++);
			ft_move_character(game_info, next_x, next_y);
